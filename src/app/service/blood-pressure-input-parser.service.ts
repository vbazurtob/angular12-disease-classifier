        ...
        try {
            for (let str of splittedInputObjStr) {
                ...
                if (m !== undefined) {
                    ...
                    if (!Number.isInteger(parseInt(sysBp))) {
                        // return {error: 'There are non-integer SysBp values. Check format'};
                        errorMsg = 'There are non-integer SysBp values. Check format';
                        break;
                    }

                    if (!Number.isInteger(parseInt(diaBp))) {
                        // return {error: 'There are non-integer DyaBp values. Check format'};

                        errorMsg = 'There are non-integer DyaBp values. Check format';
                        break;
                    }
                    ...
                    str = str.replace(/SysBP/g, '"SysBP"');
                    str = str.replace(/DiaBP/g, '"DiaBP"');
